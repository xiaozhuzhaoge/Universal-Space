...
public class SpaceSceneSwitcherDemoGUI : MonoBehaviour {
	...
	void OnGUI() {
		...
		TargetCameraFollow _cameraFollow = Camera.mainCamera.GetComponent<TargetCameraFollow>();
		if (GUI.Button(new Rect(10,70,180,25), "Camera " + _cameraFollow.followMode.ToString())) {			
			if (_cameraFollow.followMode == TargetCameraFollow.FollowMode.CHASE) {
				// Set following spectator mode
				_cameraFollow.followMode = TargetCameraFollow.FollowMode.SPECTATOR;
			} else {
				// Set chase view mode
				_cameraFollow.followMode = TargetCameraFollow.FollowMode.CHASE;
			}
		}
